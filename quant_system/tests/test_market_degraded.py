...
def test_fetch_market_snapshot_bulk_fail_watchlist_fallback():
    ...
    with patch.object(api, "fetch_spot_all", side_effect=RuntimeError("bulk fail")):
        with patch.object(api, "fetch_spot_map", return_value={
    ...
    assert "bulk_spot_failed" in data["limitations"]
