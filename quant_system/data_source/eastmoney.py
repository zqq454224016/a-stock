...
class EastMoneyCrawler(BaseCrawler):
    ...
    def _em_call(self, label: str, fn):
    ...
    def fetch_spot_all(self) -> pd.DataFrame:
        return self._retry(lambda: self._em_call("spot", self.ak.stock_zh_a_spot_em))

    def fetch_daily_hist(self, symbol: str, adjust: str = "qfq") -> pd.DataFrame:
        code = symbol.replace("sh", "").replace("sz", "").replace("bj", "")
        return self._retry(
            lambda: self._em_call(
                "daily",
    ...
    def fetch_indices(self) -> list[dict[str, Any]]:
        index_df = self._retry(lambda: self._em_call("indices", self.ak.stock_zh_index_spot_em))
        indices = []
    ...
    def fetch_industries(self) -> list[dict[str, Any]]:
        industry_df = self._retry(lambda: self._em_call("industries", self.ak.stock_board_industry_name_em))
        industries = [
    ...
    def fetch_fund_flow(self) -> tuple[dict[str, float], str | None]:
        ...
        if not is_eastmoney_disabled():
            try:
                summary_df = call_with_retry(
                    lambda: self._em_call("north_flow", self.ak.stock_hsgt_fund_flow_summary_em),
                    retries=1,
                ...
                self.log_fail(f"北向资金不可用: {humanize_fetch_error(e)}")
        ...
        try:
            ...
            self.log_fail(f"主力资金不可用: {humanize_fetch_error(e)}")
